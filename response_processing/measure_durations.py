...
def main():
    ...
    labelers = load_by_labeler(args.dumpfile)

    # print the information
    print("{:>8s}, {:>14s}, {:>7s}, {:>7s}, {:>6s}, {:>34s}, {:>26s}".format("# trials", "total time", "mean", "median", "stddev", "labeler id", "stamp"))
    for labeler_id, trials_by_labeler in labelers.items():
        total_time_s = 0
        trial_times = []
        for trial in trials_by_labeler:
        ...
        stamp = trials_by_labeler[0]["stamp"]
        print("{:8d}, {:14s}, {:7.3f}, {:7.3f}, {:.3f}, {:34s}, {:26s}".format(num_trials, total_time_formatted, mean_time_s,
                                                                         median_time_s, deviation, labeler_id,
                                                                         stamp))
        if args.verbose:
            print("total labelers:", len(trials_by_labeler))
            for trial in trials_by_labeler:
                trial_time_s = float(trial["data"]["duration_seconds"])
